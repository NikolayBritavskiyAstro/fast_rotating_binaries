...
class data(object):
    ...
    def _loadFile(self, filename, max_num_lines=-1, cols=[], final_lines=-1):
        ...
        head = pandas.read_csv(filename, delim_whitespace=True, header=1, nrows=1,on_bad_lines='skip')

        if max_num_lines > 0:
            data = pandas.read_csv(
                filename, delim_whitespace=True, header=4, nrows=max_num_lines,on_bad_lines='skip'
            )
        else:
            data = pandas.read_csv(filename, delim_whitespace=True, header=4,on_bad_lines='skip')
