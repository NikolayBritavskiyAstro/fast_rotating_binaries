...
for ii in range(0,g):
	...
	if (gon_rot[ii][9] == 'nan'):
		...
		if not math.isnan(float(gon_rot[ii][15])):
			...
			if (float(gon_rot[ii][15]) > 20) and (float(gon_rot[ii][15]) < 30) and (float(gon_rot[ii][8]) < 200) and (gon_rot[ii][17]=='SB1'):
				#print(gon_rot[ii][1])
...
for i in range(len(listaMasas)):
    ...
    if i==2:
        plt.text(float(ZAMS[i+2][0][0])-2.5,float(ZAMS[i+2][1][0])-0.67,'$\it{M}_\mathrm{\odot}$',fontsize=20)
...
plt.plot([10**log_Teff_60[i_zams_60]/1000, 10**log_Teff_50[i_zams_50]/1000],[ LOGL_60[i_zams_60], LOGL_50[i_zams_50]],linestyle='-',color='gray')

plt.plot([10**log_Teff_50[i_zams_50]/1000, 10**log_Teff_40[i_zams_40]/1000],[ LOGL_50[i_zams_50], LOGL_40[i_zams_40]],linestyle='-',color='gray')

plt.plot([10**log_Teff_40[i_zams_40]/1000, 10**log_Teff_30[i_zams_30]/1000],[ LOGL_40[i_zams_40], LOGL_30[i_zams_30]],linestyle='-',color='gray')

plt.plot([10**log_Teff_30[i_zams_30]/1000, 10**log_Teff_25[i_zams_25]/1000],[ LOGL_30[i_zams_30], LOGL_25[i_zams_25]],linestyle='-',color='gray')

plt.plot([10**log_Teff_25[i_zams_25]/1000, 10**log_Teff_20[i_zams_20]/1000],[ LOGL_25[i_zams_25], LOGL_20[i_zams_20]],linestyle='-',color='gray')

plt.plot([10**log_Teff_20[i_zams_20]/1000, 10**log_Teff_10[i_zams_10]/1000],[ LOGL_20[i_zams_20], LOGL_10[i_zams_10]],linestyle='-',color='gray')

plt.plot([10**log_Teff_10[i_zams_10]/1000, 10**log_Teff_7[i_zams_7]/1000],[ LOGL_10[i_zams_10], LOGL_7[i_zams_7]],linestyle='-',color='gray')
...
plt.plot(teff_sb1_2023,l_real_sb1_2023,color='red',marker='o',label='SB1, N23+, present work',linestyle='None',markersize=13)
...
plt.plot(teff_run,l_real_run,marker='o',linestyle='None',markersize=15,markerfacecolor='none',color='red',label='Runaways')
